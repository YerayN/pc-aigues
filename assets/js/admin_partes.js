...
// 1. CARGA LA LISTA EN PANTALLA (VISUALIZACIÓN)
async function cargarPartes() {
    const contenedor = document.getElementById('contenedor-partes');
    const filtroFecha = document.getElementById('filtro-fecha').value;
    const filtroTipo = document.getElementById('filtro-tipo').value;

    contenedor.innerHTML = '<p class="text-center text-gray-400 py-10">Buscando expedientes...</p>';

    // Ordenar por ID descendente (el más nuevo arriba)
    let query = sb
    ...
    if (!partes || partes.length === 0) {
        contenedor.innerHTML = `<div class="text-center py-10 bg-white rounded-xl border border-dashed border-gray-300"><p class="text-gray-400 italic">No se encontraron partes.</p></div>`;
        return;
    }
...
// 2. FUNCIÓN PARA ABRIR/CERRAR (LA QUE FALTABA)
...
// 3. IMPRESIÓN LIMPIA (EN VENTANA NUEVA)
// 3. IMPRESIÓN "PREMIUM" (CON LOGO Y FUENTE MODERNA)
async function imprimirPorId() {
    const id = prompt("Introduzca el NÚMERO DE EXPEDIENTE a imprimir:");
    if (!id) return;
    ...
    if (error || !parte) {
        alert("❌ No se encuentra el parte #" + id);
        return;
    }
    ...
    // URL DEL LOGO (Puedes cambiarla por la tuya propia si tienes una URL de tu escudo específico)
    const logoUrl = "../assets/img/escudo.png";

    // C. Generamos documento limpio
    ...
    ventana.document.write(`
        ...
        <head>
            ...
            <style>
                /* FUENTE MODERNA Y LIMPIA (Arial / Helvetica) */
                body { 
                ...
                .header-container {
                    display: flex;
                    align-items: center;
                    border-bottom: 2px solid #2c3e50; /* Línea azul oscura elegante */
                    padding-bottom: 20px;
                    margin-bottom: 30px;
                    gap: 20px;
                }
                ...
                /* CAJAS DE TEXTO ESTILO "FORMULARIO" */
                .box { 
        ...
        <body>
            ...
            <div class="info-bar">
                <div>
                    ...
                    <div class="value">${parte.tipo.toUpperCase()}</div>
                </div>
                <div style="text-align: right;">
                    <div class="label">FECHA Y HORA DEL SUCESO</div>
                    <div class="value">${dia} &bull; ${hora}h</div>
                </div>
            </div>

            <div class="section">
                <div class="label">UBICACIÓN EXACTA</div>
                <div class="value" style="font-size: 15px; margin-bottom: 5px;">📍 ${parte.ubicacion}</div>
            </div>

            <div class="section">
                <div class="label">DESCRIPCIÓN DE LOS HECHOS</div>
                <div class="box">"${parte.descripcion}"</div>
            </div>

            ${parte.victimas_info ? `
            <div class="section">
                <div class="label label-red">DATOS DE AFECTADOS / VÍCTIMAS</div>
                <div class="box box-red"><strong>⚠️ ATENCIÓN:</strong>\n${parte.victimas_info}</div>
            </div>` : ''}

            <div class="section">
                <div class="label">ACTUACIÓN REALIZADA</div>
                <div class="box" style="background: #fdfdfd;">${parte.acciones_realizadas}</div>
            </div>

            <div style="display: flex; gap: 20px;">
                <div class="section" style="flex: 1;">
                    <div class="label">RECURSOS UTILIZADOS</div>
                    <div style="border-bottom: 1px solid #eee; padding: 5px 0; font-size: 14px;">${parte.recursos_usados || 'No especificado'}</div>
                </div>
                <div class="section" style="flex: 1;">
                    <div class="label">COLABORACIÓN CON OTROS ORGANISMOS</div>
                    <div style="border-bottom: 1px solid #eee; padding: 5px 0; font-size: 14px;">${parte.organismos_presentes || 'Sin colaboración externa'}</div>
                </div>
            </div>

            <div class="footer">
                <div>
                    <div class="label">VOLUNTARIO INFORMANTE</div>
                    <div class="value" style="text-transform: uppercase; font-size: 14px;">${parte.usuario_creador}</div>
                    <div style="font-size: 11px; color: #888; margin-top: 4px;">Agrupación de Voluntarios de Protección Civil</div>
                </div>
                <div class="firma-box">
                    Firma, Sello y Fecha - Coordinador Jefe
                </div>
            </div>
